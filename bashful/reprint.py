...
def line_width(line):
    ...
    return result - (len(line) - ansi_len(line))
...
def print_multi_line(content, force_single_line, sort_key):
    ...
    if isinstance(content, list):
        ...
        for k, v in content.items():
            _k, _v = map(preprocess, (k, v))
            print_line("{:10s}: {}".format(_k, _v), columns, force_single_line)
    else:
        raise TypeError("Excepting types: list, dict. Got: {}".format(type(content)))
...
class output:
    ...
    def __init__(self, output_type="list", initial_len=1, interval=0.1, force_single_line=False, no_warning=False, sort_key=lambda x:x[0]):
        ...
        if output_type == "list":
            ...
            self.warped_obj = output.SignalOrderedDict(self, collections.OrderedDict())
